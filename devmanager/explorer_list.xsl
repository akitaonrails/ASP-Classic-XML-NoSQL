...
<xsl:template match="parent">
  <tr> 
    <td height="16" colspan="4"> 
      ...
      <a href="#" onclick="ClickFolder( '..' )"><xsl:value-of select="path"/></a></td>
  </tr>
</xsl:template>
...
<xsl:template match="folder">
  <tr> 
    ...
    <xsl:choose>
      ...
      &nbsp;
      <a href="#" onclick="ClickFolder( '{ translate( path, '\', '/' ) }' )"><xsl:value-of select="name"/></a></td>
...
<xsl:template match="file">
  <tr> 
	...
	<a href="#" onclick="ClickFile( '{ translate( path, '\', '/' ) }' )"><xsl:value-of select="name"/></a>
    </td>
    <td align="right"><xsl:value-of select="size"/></td>
    <td align="right"><xsl:value-of select="created"/></td>
    <td align="right"><xsl:value-of select="lastmodified"/></td>
